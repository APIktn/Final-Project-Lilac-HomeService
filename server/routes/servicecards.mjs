...
//-------Get Top Services------//
servicesRouter.get("/top6services", async (req, res) => {
  try {
    ...
    if (detailedData) {
      detailedData.forEach((service) => {
        if (Array.isArray(service.service_list)) {
          service.total_quantity = service.service_list.reduce(
      ...
      detailedData = detailedData.slice(0, 6); // Reassigning 'detailedData' is causing the error

      // Calculate totalQuantity for the top 3 services
      let totalQuantity = detailedData.reduce(
        (sum, service) => sum + service.total_quantity,
        0
      );
